...
public class dodo {
	  ...
	  static void ffmpegmontagemaker(String ffmpegcommand){
		  try {
			    Process process = new ProcessBuilder("cmd.exe", "/c", ffmpegcommand).start();
			} catch (IOException e) {
			    e.printStackTrace();
			}
	  }
    ...
    //System.out.println(outputBuilder.toString());
    	finalffmpegCommand.append(" -filter_complex \""+outputBuilder.toString().substring(0,(outputBuilder.length()-1))+"\" -c:v h264_nvenc -b:v 60M -bf 2 -preset slow -c:a aac -map \"["+"vclip"+(clipduration.length-1)+"]\" -map \"["+"aclip"+(clipduration.length-1)+"]\" -movflags +faststart "+"\""+folderLocation+"\\out.mp4"+"\"");
    	System.out.println(finalffmpegCommand);
    	ffmpegmontagemaker("dir /b");
        /*try {
            ffmpegmontagemaker("dir /b");
        } catch (IOException e) {
